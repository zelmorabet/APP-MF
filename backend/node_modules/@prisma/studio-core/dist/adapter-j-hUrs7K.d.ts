...
interface StudioQueryInsightPrismaQueryInfo {
    action: string;
    isRaw: boolean;
    model?: string;
    payload?: Record<string, unknown> | Array<Record<string, unknown>>;
}
interface StudioQueryInsightQuery {
    count: number;
    duration: number;
...
interface StudioQueryInsightsSnapshot {
    generatedAt: number;
    pollingIntervalMs?: number;
    queries: StudioQueryInsightQuery[];
}
interface StudioQueryInsightsSnapshotRequest {
    limit?: number;
    since?: number;
}
interface StudioQueryInsights {
    getSnapshot(request: StudioQueryInsightsSnapshotRequest, options?: ExecuteOptions): Promise<Either<Error, StudioQueryInsightsSnapshot>>;
}
