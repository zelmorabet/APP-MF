...
var chunk_PPPJF3PE_exports = {};
__export(chunk_PPPJF3PE_exports, {
  downloadZip: () => downloadZip,
  require_temp_dir: () => require_temp_dir
});
module.exports = __toCommonJS(chunk_PPPJF3PE_exports);
...
async function fetchChecksum(url) {
  try {
    ...
    const response = await fetch(checksumUrl, {
      agent: (0, import_chunk_QTVXEC6H.getProxyAgent)(url)
    });
    if (!response.ok) {
...
async function downloadZip(url, target, progressCb) {
  ...
  await (0, import_chunk_D2GOUA3T.overwriteFile)(partial, target);
  try {
