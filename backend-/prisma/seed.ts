import { PrismaClient, TypeContribution, GroupeAge, StatutPresence } from '@prisma/client';
...
async function main() {
  ...
  const rsge = await prisma.rsge.upsert({
    ...
    create: {
      ...
      prenom: 'Marie',
      nom: 'Tremblay',
      // NAS placeholder — sera chiffré en production
  ...
  const parent1 = await prisma.parent.upsert({
    ...
    create: {
      ...
      ville: 'Montréal',
      codePostal: 'H2B 2B2',
      typeContribution: TypeContribution.REDUIT,
    },
  });
